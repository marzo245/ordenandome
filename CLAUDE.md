...
**Importación de KO altas** (`POST /api/ko/import`, usa `xlsx`/SheetJS server-side): el Excel trae cada cuenta atascada como una fila. Lee la hoja **`default_1`** (si no existe, la de más filas; la otra hoja `Hoja1` es una tabla dinámica) y cruza el campo **`Error normalizado`** contra `ko_entries.error` **por contención** (la frase del KO está contenida en el texto del caso) — **sin IA, para no gastar tokens**. 1 match = conocida; 0 o ambiguo (>1) = desconocida. Autodetecta la columna del error (si no puede, `{ needsColumn, columnas }` y el cliente la elige). Cada caso guarda `error_texto` (el «Error normalizado») para **agrupar**: «Conocidas» agrupa por KO (worklist con plan de acción + resolver en lote), «Pendientes» agrupa por error para normalizar/promover **todo el grupo de golpe**. Promover/vincular en lote: `POST /api/ko/casos/promover` (`{ mode: 'link'|'create', caso_ids, ... }`, `create` reusa `EntryEditor`); acciones masivas: `POST /api/ko/casos/bulk` (`resolver`|`reabrir`|`descartar`); caso individual: `/api/ko/casos/[id]`; lista `/api/ko/casos`; lotes `/api/ko/lotes/[id]` (cascade).
