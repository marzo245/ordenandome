...
**Importación de KO altas** (`POST /api/ko/import`, usa `xlsx`/SheetJS server-side): el Excel trae cada cuenta atascada como una fila. Lee la hoja **`default_1`** (si no existe, la de más filas; la otra hoja `Hoja1` es una tabla dinámica). **El ÚNICO criterio de cruce es el `Error normalizado`** (lo que el usuario normaliza y mantiene): se cruza por contención contra `ko_entries.error` — **sin IA** y **sin mirar el ECO_Notes**. ⚠️ Decisión del usuario (firme): **todas las filas con el mismo `Error normalizado` reciben el MISMO veredicto** — NO se separan por el mensaje crudo. Coincidencia única = conocida; 0 o ambiguo (>1) = desconocida. El `Error normalizado` se guarda en `error_texto` (también es la etiqueta para agrupar las pendientes). Consecuencia: errores cuyo enunciado normalizado es genérico (p. ej. "Error creando el Medidor", que en el catálogo existe como varios KOs con código: `EN 244`, `EL 105`…) quedan como UN grupo pendiente; el usuario lo promueve a un solo KO. En el Excel real: 741 conocidas / 401 pendientes. La lógica de cruce está en `src/lib/ko-match.ts` (`normKoError`, `buildKoIndex`, `createKoMatcher`), reutilizada por el import y por **`POST /api/ko/casos/recruzar`** (botón «Re-cruzar pendientes»), que reevalúa las pendientes contra el catálogo actual SIN re-subir el Excel — necesario porque el cruce se calcula al importar, así que tras crear/editar un KO hay que re-cruzar (o re-importar) para que las pendientes que ahora cruzan pasen a conocidas. El «Error normalizado» se guarda en `error_texto` solo como **etiqueta para agrupar** las pendientes. Autodetecta la columna de cruce (ECO_Notes; si no, `{ needsColumn, columnas }`). «Conocidas» agrupa por KO (worklist con plan de acción + resolver en lote; cada cuenta abre un modal de gestión con estado interno [por gestionar/en revisión/resuelto], incidencia [nº + estado pendiente→enviado→ok] e **historial** automático de cambios), «Pendientes» agrupa por error para promover **todo el grupo de golpe**. Endpoints: `POST /api/ko/casos/promover` (`{ mode: 'link'|'create', caso_ids, ... }`, `create` reusa `EntryEditor`), `POST /api/ko/casos/bulk` (`resolver`|`reabrir`|`descartar`), `/api/ko/casos[/[id]]`, `/api/ko/lotes` (GET) y `/api/ko/lotes/[id]` (DELETE cascade). Ambas pestañas tienen buscador (Conocidas además filtra por clasificación) y la zona de subida lista las importaciones con botón Borrar.
