...
Tablas clave: `tasks`, `notes`/`note_links`, `sistemas`, `sistema_secciones` (acciones de cada sistema; columna `pasos` jsonb = flujo multi-sistema), `ko_entries`/`ko_subprocesos` (catálogo + subprocesos), `ko_import_lotes`/`ko_import_casos` (importación de Excel de KO altas: un lote por subida, un caso por fila; `fila` jsonb = la fila cruda, `tipo` conocida|desconocida = normalización, `estado` pendiente|en_revision|resuelto = gestión interna, `incidencia_numero`/`incidencia_estado` (pendiente|enviado|ok), `historial` jsonb = bitácora de cambios), `github_activity`, `daily_summaries`.

**Importación de KO altas** (`POST /api/ko/import`, usa `xlsx`/SheetJS server-side): el Excel trae cada cuenta atascada como una fila.

- **Hoja:** lee `default_1` (si no existe, la de más filas; `Hoja1` suele ser una dinámica de resumen, se ignora).
- **Cruce:** criterio PRIMARIO el campo **`Error normalizado`** (lo que él normaliza y mantiene), por contención contra `ko_entries.error` (insensible a tildes), sin IA. Coincidencia única = conocida; 0 o varias → se intenta resolver con el **`ECO_Notes`** del caso (ver abajo); si tampoco, desconocida. Si no detecta la columna del error → `{ needsColumn, columnas }` y el cliente la elige.
- **`ECO_Notes` como desempate/cruce (decisión del usuario, jun-2026):** el mensaje crudo trae el código específico que el «Error normalizado» genérico pierde. Por eso `ECO_Notes` se usa cuando el error no basta: (a) **desempate** si cruza con varios KOs → gana el KO cuyo `eco_notes` esté contenido en el crudo y sea el MÁS LARGO (más específico; p. ej. `E011-No fue posible crear el servicios` + `E137-Error insertando cuenta` → el de E137); (b) **cruce** si el error no cruza con ninguno → se busca el `eco_notes` de un KO contenido en el `ECO_Notes` del caso (genérico "Error creando la Cuenta Contrato" → EM 101/EM 102/AR 129 según el crudo). Ver [[ko-import-cruce-por-error-normalizado]].
- Consecuencia: un enunciado genérico (p. ej. "Error creando el Medidor", que en el catálogo existe como varios KOs con código `EN 244`, `EL 105`…) queda como UN grupo pendiente; el usuario crea un KO "a secas" y lo promueve. En el Excel real: 741 conocidas / 401 pendientes.
- Lógica en `src/lib/ko-match.ts` (`normKoError`, `buildKoIndex`, `createKoMatcher`), reutilizada por el import y por **`POST /api/ko/casos/recruzar`** (botón «Re-cruzar pendientes»): el cruce se calcula AL IMPORTAR, así que tras crear/editar un KO hay que re-cruzar (o re-importar) para que las pendientes que ahora cruzan pasen a conocidas.
- **Conocidas** (pestaña): worklist agrupada por KO con su plan de acción; resolver en lote o una a una. Cada cuenta abre un **modal de gestión** con estado interno (`Por gestionar → En revisión → Resuelto`), **incidencia** opcional (`incidencia_numero` + `incidencia_estado`: `pendiente → enviado → ok`) e **historial** automático de cambios. El `error_texto` (= «Error normalizado») se usa para agrupar.
- **Pendientes** (pestaña): agrupadas por error; promover a KO nuevo (reusa `EntryEditor`) o vincular a uno existente (combobox con búsqueda) o descartar, por grupo.
- Endpoints: `POST /api/ko/casos/promover` (`{ mode:'link'|'create', caso_ids, ... }`), `POST /api/ko/casos/bulk` (`resolver`|`reabrir`|`descartar`), `POST /api/ko/casos/recruzar`, `GET/PATCH/DELETE /api/ko/casos[/[id]]`, `GET /api/ko/lotes`, `DELETE /api/ko/lotes/[id]` (cascade). Ambas pestañas tienen buscador (Conocidas además filtra por clasificación) y la zona de subida lista las importaciones con botón Borrar.
...
src/lib/
  ...
  ko-match.ts         # cruce de la importación de KO altas (Error normalizado → catálogo)
