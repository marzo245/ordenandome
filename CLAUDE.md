...
- **Cruce — ⚠️ decisión firme del usuario:** el ÚNICO criterio es el campo **`Error normalizado`** (lo que él normaliza y mantiene), por contención contra `ko_entries.error`, **sin IA y sin mirar el `ECO_Notes`**. **Todas las filas con el mismo `Error normalizado` reciben el MISMO veredicto** (no se separan por el crudo). Coincidencia única = conocida (vinculada a ese KO); 0 o ambigua (>1) = desconocida. Si no detecta la columna → `{ needsColumn, columnas }` y el cliente la elige. Ver [[ko-import-cruce-por-error-normalizado]].
