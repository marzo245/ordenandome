...
**Importación de KO altas** (`POST /api/ko/import`, usa `xlsx`/SheetJS server-side): el Excel trae cada cuenta atascada como una fila. Lee la hoja **`default_1`** (si no existe, la de más filas; la otra hoja `Hoja1` es una tabla dinámica) y **cruza por contención contra `ECO_Notes` (el mensaje crudo)** — NO contra el «Error normalizado» — porque el ECO_Notes incluye el **código del error** (p. ej. `EN 244`) que distingue KOs con el mismo enunciado pero distinta clasificación (varios "Error creando el Medidor": SAP-004/Sistemas `EM 111`, ECO-009/Relanzamiento `EN 244`…). Una cuenta es conocida si la frase de UN KO (`ko_entries.error`, que en el catálogo incluye ese código) está contenida en su ECO_Notes — **sin IA**. 1 match = conocida; 0 o ambiguo (>1) = desconocida. **Doble pasada:** si el ECO_Notes no cruza (a veces el crudo intercala valores en medio de la frase, p. ej. "tipo de documento CC es erroneo", y rompe la contención), cae al «Error normalizado» (solo coincidencia única). En el Excel real: ECO_Notes 982 → doble pasada 1016 conocidas. El «Error normalizado» se guarda en `error_texto` solo como **etiqueta para agrupar** las pendientes. Autodetecta la columna de cruce (ECO_Notes; si no, `{ needsColumn, columnas }`). «Conocidas» agrupa por KO (worklist con plan de acción + resolver en lote), «Pendientes» agrupa por error para promover **todo el grupo de golpe**. Endpoints: `POST /api/ko/casos/promover` (`{ mode: 'link'|'create', caso_ids, ... }`, `create` reusa `EntryEditor`), `POST /api/ko/casos/bulk` (`resolver`|`reabrir`|`descartar`), `/api/ko/casos[/[id]]`, `/api/ko/lotes` (GET) y `/api/ko/lotes/[id]` (DELETE cascade). Ambas pestañas tienen buscador (Conocidas además filtra por clasificación) y la zona de subida lista las importaciones con botón Borrar.
